...
import { getLocalStorage, setLocalStorage } from '../Services/weatherService';
export default function useWeather() {
  const [history, setHistory] = useState(() => {
    const saved = getLocalStorage("weather_history");
    return saved ? saved : [];
  });
  ...
  const addToHistory = (targetCity) => {
    ...
    setLocalStorage("weather_history", newHistory);
  };
  ...
  const deleteHistoryItem = (cityToDelete) => {
    ...
    setLocalStorage("weather_history", updatedHistory);
  };
